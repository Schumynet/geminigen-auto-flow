...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function startAutoFlow() {
    ...
    let characterId = selectedCharacter;
    let environmentId = selectedEnvironment;
    
    // AUTO-GENERATE: se non selezionati, genera automaticamente
    if (!characterId || !environmentId) {
      showStatus('🧠 Auto-generazione personaggio/ambiente...');
      
      const autoResp = await sendMsg({ 
        type: 'AUTO_GENERATE_MISSING', 
        scenes: scenes 
      });
      
      if (autoResp.success && autoResp.autoGenerated) {
        characterId = autoResp.character?.id || characterId;
        environmentId = autoResp.environment?.id || environmentId;
        
        // Aggiorna UI con gli elementi generati
        if (autoResp.character) {
          selectedCharacter = autoResp.character.id;
          updateCharacterDisplay();
          // Aggiungi alla lista
          characters[autoResp.character.id] = autoResp.character;
          renderCharacters();
        }
        
        if (autoResp.environment) {
          selectedEnvironment = autoResp.environment.id;
          updateEnvironmentDisplay();
          environments[autoResp.environment.id] = autoResp.environment;
          renderEnvironments();
        }
        
        showStatus(`✅ Auto-generato: ${autoResp.character?.name || 'Personaggio'} + ${autoResp.environment?.name || 'Ambiente'}`);
      }
    }
  ...
  function updateCharacterDisplay() {
    const charAutoBadge = $('charAutoBadge');
    
    if (selectedCharacter && characters[selectedCharacter]) {
      const char = characters[selectedCharacter];
      const isAuto = char.autoGenerated;
      ...
      if (isAuto) {
        charAutoBadge.style.display = 'inline';
      } else {
        charAutoBadge.style.display = 'none';
      }
    } else {
      selectCharacter.innerHTML = `
        <div class="select-avatar">👤</div>
        <div class="select-info">
          ...
          <div class="select-desc">Verrà generato automaticamente</div>
        </div>
      `;
      charAutoBadge.style.display = 'none';
    }
  ...
  function updateEnvironmentDisplay() {
    ...
    if (selectedEnvironment && environments[selectedEnvironment]) {
      ...
      if (isAuto) {
        envAutoBadge.style.display = 'inline';
      } else {
        envAutoBadge.style.display = 'none';
      }
    } else {
      selectEnvironment.innerHTML = `
        <div class="select-avatar">🏠</div>
  ...
  function showStatus(message) {
    ...
    let statusEl = document.getElementById('popup-status');
    if (!statusEl) {
      statusEl = document.createElement('div');
      statusEl.id = 'popup-status';
