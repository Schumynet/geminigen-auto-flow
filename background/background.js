...
const ScriptParser = {
  // Parse copione - Ogni riga = un prompt/scena
  parseScript(text) {
    const scenes = [];
    ...
    const isSceneStart = (line) => {
      const trimmed = line.trim();
      for (const p of scenePatterns) {
        if (p.test(trimmed)) return true;
      }
      // Se la riga è solo titolo (senza contenuto dopo)
      if (/^(SCENA|SCENE|ACT|PARTE|PART)\s*[\:\.\)]?\s*$/i.test(trimmed)) return true;
      return false;
    };
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Linea vuota = separa prompt precedenti
      if (isEmpty(line)) {
        if (promptBuffer.length > 0 && currentScene) {
          const promptText = promptBuffer.join(' ').trim();
          if (promptText.length > 2) {
            currentScene.prompts.push(promptText);
          }
          promptBuffer = [];
        }
        continue;
      ...
      if (isSceneStart(trimmed)) {
        // Salva scena precedente
        if (currentScene && currentScene.prompts.length > 0) {
        ...
        // È contenuto - aggiungi a prompt buffer
        if (!currentScene) {
          currentScene = {
            index: scenes.length,
            title: `Prompt ${scenes.length + 1}`,
            prompts: [],
            duration: 6,
            character: null,
            environment: null
    ...
    // Se ancora nessuna scena, ogni riga non vuota = scena
    if (scenes.length === 0) {
    ...
    console.log(`[ScriptParser] Parsed ${scenes.length} scenes/prompts`);
    return scenes;
  },
